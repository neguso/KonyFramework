...
function IdentityService(konyRef) {
    ...
    this.login = function(options, successCallback, failureCallback) {
        ...
        function invokeAjaxCall(url, params, headers) {
            ...
            if (serviceObj.isAnonymousLogin) {
...
kony.sdk.prototype.getSyncService = function() {
    ...
    var tempStartSession = sync.startSession;
    var tempPerformUpgrade = sync.performUpgrade;
    var tempIsUpgradeRequired = sync.isUpgradeRequired;
    ...
    function tempStartSession1(config) {
        tempStartSession(tempConfig(config));
    }

    function tempPerformUpgrade1(config) {
        tempPerformUpgrade(tempConfig(config));
    }

    function tempIsUpgradeRequired1(config) {
        tempIsUpgradeRequired(tempConfig(config));
    }
    konySync.startSession = tempStartSession1;
    konySync.performUpgrade = tempPerformUpgrade1;
    konySync.isUpgradeRequired = tempIsUpgradeRequired1;
    return konySync;
}
...
function konyNetHttpRequest(url, params, headers, successCallback, failureCallback) {
    ...
    function localRequestCallback(result) {
        ...
        logger.log("readyState is " + readyState);
    ...
    for (var key in params) {
        if (params[key]) {
            paramsTable.append(key, toString(params[key]));
        }
    }
    if (headers) {
