var toolkit = (function() {
        ...
        if (typeof config === 'function') {
            ...
            if (len === 1) {
                ...
                return {
                    instance: null,
                    get: function() {
                        var dependencies = config.slice(0, len - 1).map(function(currentValue) {
                            return injector.resolve(currentValue);
                        });
                        return config[len - 1].apply(null, dependencies.map(function(serviceProvider) {
                            return injector.instance(serviceProvider);
                        }));
                    }
                };
            }
            ...
            if (len === 1) {
                ...
                return {
                    get: function() {
                        ...
                        var o = {};
                        config[len - 1].apply(o, dependencies.map(function(serviceProvider) {
                            return injector.instance(serviceProvider);
                        }));
                        return o;
                    }
    ...
    var _toolkit = {
            ...
            if (arguments.length === 1) {
                ...
                var _module = {
                    ...
                    _resolve: function(name) {
                        if (services.hasOwnProperty(name)) return services[name];
                        return null;
                    },
                    _run: function() {
                        run.forEach(function(serviceConfig) {
                            getServiceProvider(serviceConfig, _toolkit.injector(requires.concat([moduleName]))).get();
                        });
                    }
                };
            ...
            var _injector = {
                ...
                resolve: function(service) {
                    ...
                    for (var i = 0; i < modules.length; i++) {
                        ...
                        if (serviceProvider != null) found.push(serviceProvider);
                    }
                    if (found.length === 0) {
        ...
        start: function() {
            _toolkit._modules().forEach(function(module) {
                _toolkit.module(module)._run();
            });
        },
        _modules: function() {
            var ary = [];
            for (var prop in modules)
            if (modules.hasOwnProperty(prop)) ary.push(prop);
            return ary;
        }
    };
    return _toolkit;
