toolkit.module('kony', ['system']).run([function() {}]).factory('$state', ['$injector', function($injector) {
    var _state = {
        states: {},
        register: function(stateName, form, controllerName) {
            if (_state.states.hasOwnProperty(stateName)) throw new Error('Duplicate state "' + stateName + '".');
            // store state
            ...
            toolkit.module('kony').service('$scope', function() {
                ...
                this.on = function() {
                    if (arguments.length < 2 || arguments.length > 3) throw new Error('Invalid arguments.');
                    ...
                    if (event === 'init') this.form[event] = callback;
                    else attachEvent(widget, event, callback);
                };
            });
    ...
    var _location = {
        ...
        startup: function(state) {
            if (arguments.length === 0) {
                if (_location.history.length > 0) return _location.history[0].state;
                else return null;
        ...
        back: function() {
            if (_location.history.length < 2) return;
            pop();
    ...
    this.timeout = function(t) {
        if (arguments.length === 0) return _timeout;
        else if (arguments.length === 1) _timeout = t;
        else throw new Error('Invalid arguments.');
    };
    /* Cancel last service call. */
    this.cancel = function() {
        if (connections.length > 0) kony.net.cancel(connections.pop());
    };
            ...
            if (status === 100) {
                ...
                try {
                    if (results.httpresponse.responsecode === 200) {
                        ...
                        // error
                        deferred.reject({
                            code: results.httpresponse.responsecode,
                            message: results.errmsg
                        });
                    }
                } finally {
    ...
    this.kony = function(url, app, service, params) {
        ...
        if (params.hasOwnProperty(key)) inputParamTable[key] = params[key];
        var deferred = RSVP.defer();
        var conn = kony.net.invokeServiceAsync(
        url, inputParamTable, function(status, results) {
            if (status === 100) {
                // network call initiated successfully
            } else if (status === 200) {
                // network is in progress, start receiving the result
