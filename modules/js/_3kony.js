...
Event.prototype.fire = function(args) {
	this.handlers.forEach(function(handler) { handler.call(null, args); });
};
...
function ObjectListener(func, model, properties)
{
	...
	// constructor
	if(typeof model.PropertyChanged === 'undefined')
		throw new Error('Model doesn\'t implement INotifyPropertyChanged interface.');

	var self = this;
	if(properties.length > 0)
		model.PropertyChanged.attach(function(eventArgs) { if(!self.suspended && properties.indexOf(eventArgs.propertyName) != -1) self.callback(eventArgs); });
}
...
function WidgetListener(func, widget, events)
{
	...
	events.forEach(function(event) {
		self.attachEvent(self.widget, event, function() {
			if(!self.suspended)
				self.handleEvent.apply(self, [event].concat(arguments));
		});
	});
}
...
TextBoxListener.prototype.handleEvent = function(event)
{
	if(event === 'onTextChange')
		this.callback({ propertyName: 'text', newValue: this.widget.text, oldValue: null });
};
...
function BindingManager()
{
	...
	this.perform = function()
	{
		this.bindings.forEach(function(binding) { binding.perform(); });
	};
}
